...
def generate_segmentations_monai(data_loader, model, writer_1, args):
    ...
    for idx, val_data in enumerate(data_loader):
        ...
        segs = torch.zeros((1, 4, ref_seg.shape[0], ref_seg.shape[1], ref_seg.shape[2]))
...
#        ed = np.logical_and(segs[2], np.logical_not(segs[1]))
        ...
        labelmap[net] = 1
        labelmap[ed] = 2
        labelmap_seg=labelmap
        labelmap = sitk.GetImageFromArray(labelmap)


        segmap_et = labelmap_seg == 4
        segmap_tc = np.logical_or(segmap_et,  labelmap_seg == 1)
        segmap_wt = np.logical_or(segmap_tc, labelmap_seg == 2)
        segmap = np.stack([segmap_et, segmap_tc, segmap_wt])
        ...
        patient_metric_list = calculate_metrics(segmap, refmap, patient_id)
    ...
    overlap = df.boxplot(METRICS[0], by="label", return_type="axes")
